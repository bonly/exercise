...
func onStart() {
    ...
    // read pic
    f, err := app.Open("activity_layout.png");
    if err != nil {
		panic(err);
    }
    ...
    */
    ...
    wd := fmt.Sprintf("width is: %v", width);
    hg := fmt.Sprintf("height is: %v", height);
    bg := sprite.SubTex{Tex, image.Rect(0, 0, 962, 610)}; //加载进来时的大小是以px为单位
	gopher := &sprite.Node{
    ...
    addText(game, str, 12, geom.Point{ 50, 80}); 
    addText(game, wd, 12, geom.Point{ 50, 100}); 
    addText(game, hg, 12, geom.Point{ 50, 120}); 
}
