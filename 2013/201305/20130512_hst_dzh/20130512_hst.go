...
func data2hst(pre string, ifile string, ofile string){
  if len(ifile) == 0{
    return;
  }
  if len(ofile)==0{
    ofile=filepath.Dir(ifile) + "/" + pre +
  ...
  copy(head.Symbol[:], pre + strings.TrimSuffix(filepath.Base(ifile),filepath.Ext(ifile)));
  ...
  for {
    ...
    bar.Ctm = tm.Unix();
    bar.Open = float64(day.Open) / 1000.0;
    bar.High = float64(day.High) / 1000.0;
    bar.Low = float64(day.Low) / 1000.0;
    bar.Close = float64(day.Close) / 1000.0;
    // bar.Open = float64(day.Open);
    // bar.High = float64(day.High);
    // bar.Low = float64(day.Low);
    // bar.Close = float64(day.Close);    
    bar.Volume = int64(day.Turnover);
    // bar.Spread
    // bar.Real_volume
