 ...
 * 
 * gcc -std=c99 -Wall -Werror -m32 -O0 -fpic -shared -ldl -lGL -o eve.so 20130519_inject.c
 *
...
void doevil() {
	...
	//uint32_t read_uint( uint32_t addr ) { return *((uint32_t*) addr); } //32bit
	uint32_t read_uint( uint32_t addr ) { return (intptr_t) addr; } //64bit
	//float read_float( uint32_t addr ) { return *((float*) addr); } //32bit
	float read_float( uint32_t addr ) { return (intptr_t) addr; } //32bit

	// calling game functions works too! (WoW 3.3.0a)
	static int (*ClntObjMgrGetActivePlayer)() = (void*) 0x0047A2B0;

	printf("doevil(), frame %d... =============================== bonly ========= ", framecnt);
...
http://www.ownedcore.com/forums/world-of-warcraft/world-of-warcraft-bots-programs/wow-memory-editing/330135-linux-ld_preload-injection-without-patching-wine.html
*/
