...
// USER_API参数
extern CThostFtdcTraderApi* ptUserApi;

// 配置参数
extern char T_FRONT_ADDR[];		// 前置地址
extern char T_BROKER_ID[];		// 经纪公司代码
extern char T_INVESTOR_ID[];		// 投资者代码
extern char T_PASSWORD[];			// 用户密码
extern char INSTRUMENT_ID[];	// 合约代码
extern TThostFtdcPriceType	LIMIT_PRICE;	// 价格
extern TThostFtdcDirectionType	DIRECTION;	// 买卖方向

// 请求编号
extern int itRequestID;
...
void CTraderSpi::ReqUserLogin()
{
	...
	strcpy(req.BrokerID, T_BROKER_ID);
	strcpy(req.UserID, T_INVESTOR_ID);
	strcpy(req.Password, T_PASSWORD);
	int iResult = ptUserApi->ReqUserLogin(&req, ++itRequestID);
		...
		CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast && !IsErrorRspInfo(pRspInfo))
	{
		...
		///获取当前交易日
		cerr << "--->>> 获取当前交易日 = " << ptUserApi->GetTradingDay() << endl;
		///投资者结算结果确认
...
void CTraderSpi::ReqSettlementInfoConfirm()
{
	...
	memset(&req, 0, sizeof(req));
	strcpy(req.BrokerID, T_BROKER_ID);
	strcpy(req.InvestorID, T_INVESTOR_ID);
	int iResult = ptUserApi->ReqSettlementInfoConfirm(&req, ++itRequestID);
...
void CTraderSpi::ReqQryInstrument()
{
	...
	while (true)
	{
		int iResult = ptUserApi->ReqQryInstrument(&req, ++itRequestID);
...
void CTraderSpi::ReqQryTradingAccount()
{
	...
	while (true)
	{
		int iResult = ptUserApi->ReqQryTradingAccount(&req, ++itRequestID);
		if (!IsFlowControl(iResult))
		{
...
void CTraderSpi::OnRspQryTradingAccount(CThostFtdcTradingAccountField *pTradingAccount, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast && !IsErrorRspInfo(pRspInfo))
	{
		cerr << setiosflags(ios::showpoint) << "Available: " << pTradingAccount->Available << endl;
		///请求查询投资者持仓
...
void CTraderSpi::ReqQryInvestorPosition()
{
	...
	while (true)
	{
		int iResult = ptUserApi->ReqQryInvestorPosition(&req, ++itRequestID);
		if (!IsFlowControl(iResult))
		{
...
//	TThostFtdcRequestIDType	RequestID;
	...
	int iResult = ptUserApi->ReqOrderInsert(&req, ++itRequestID);
...
void CTraderSpi::ReqExecOrderInsert()
{
	...
	int iResult = ptUserApi->ReqExecOrderInsert(&req, ++itRequestID);
...
void CTraderSpi::ReqForQuoteInsert()
{
	...
	int iResult = ptUserApi->ReqForQuoteInsert(&req, ++itRequestID);
...
void CTraderSpi::ReqQuoteInsert()
{
	...
	int iResult = ptUserApi->ReqQuoteInsert(&req, ++itRequestID);
...
//	TThostFtdcUserIDType	UserID;
	...
	int iResult = ptUserApi->ReqOrderAction(&req, ++itRequestID);
...
void CTraderSpi::ReqExecOrderAction(CThostFtdcExecOrderField *pExecOrder)
{
	...
	int iResult = ptUserApi->ReqExecOrderAction(&req, ++itRequestID);
...
void CTraderSpi::ReqQuoteAction(CThostFtdcQuoteField *pQuote)
{
	...
	int iResult = ptUserApi->ReqQuoteAction(&req, ++itRequestID);
...
bool CTraderSpi::IsErrorRspInfo(CThostFtdcRspInfoField *pRspInfo)
{
	...
	if (bResult){
		char str_utf8[200];
		cerr << "--->>> ErrorID=" << pRspInfo->ErrorID << ", ErrorMsg=" << gb2utf8(pRspInfo->ErrorMsg) << endl;
	}
	return bResult;
}
