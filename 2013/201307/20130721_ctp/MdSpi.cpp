...
{
	D();CThostFtdcReqUserLoginField req;
...
void CMdSpi::OnRtnForQuoteRsp(CThostFtdcForQuoteRspField *pForQuoteRsp){
	...
	cerr << "合约代码：" << pForQuoteRsp->InstrumentID <<endl;
	
}

void CMdSpi::OnRspSubMarketData(CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	cerr << "OnRspSubMarketData: " << pRspInfo->ErrorMsg << endl;
	cerr << "InstrumentID: " << pSpecificInstrument->InstrumentID << endl;
	cerr << "nRequestID: " << nRequestID << "\t" << bIsLast << endl;
}
...
{
	cerr << "OnRspSubForQuoteRsp: " << pRspInfo->ErrorMsg << endl;
	cerr << "InstrumentID: " << pSpecificInstrument->InstrumentID << endl;
	cerr << "nRequestID: " << nRequestID << "\t" << bIsLast << endl;
...
{
	// data2hst(pDepthMarketData);
	data2db(pDepthMarketData);
	cerr << "OnRtnDepthMarketData" << endl;
	cerr << " 代码：" << pDepthMarketData->InstrumentID;
	cerr << " 交易日： " << pDepthMarketData->TradingDay << " " << pDepthMarketData->UpdateTime;
	cerr << " 最新价格： "<< pDepthMarketData->LastPrice; 
	cerr << " 成交量： " << pDepthMarketData->Volume;
	cerr << " 结算价: " << pDepthMarketData->SettlementPrice;
	cerr << " 成交金额： " << pDepthMarketData->Turnover << endl;
}
