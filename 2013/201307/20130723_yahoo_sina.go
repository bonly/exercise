...
从新浪(sina.com.cn)获取
网页输出：http://money.finance.sina.com.cn/corp/go.php/vMS_MarketHistory/stockid/000028.phtml
http://stock2.finance.sina.com.cn/futures/api/json.php/IndexService.getInnerFuturesDailyKLine?symbol=AG1507
*/
package main
...
func insertData(dt Data){
	stmt, err := db.Prepare(`
		insert into history(Code, Dt, Open, High, Low, Close, Volume)
		values (?, ?, ?, ?, ?, ?, ?)`);
	if err != nil{
		log.Fatal(err);
	}
