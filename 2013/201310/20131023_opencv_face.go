...
func main() {
	...
	image := opencv.LoadImage(path.Join(path.Dir(currentfile), "face.jpg"));

	// cascade := opencv.LoadHaarClassifierCascade(path.Join(path.Dir(currentfile), 
		// "haarcascade_frontalface_alt.xml"));//不需要绝对的全路径
