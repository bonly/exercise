...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *Entry, keys []string, timestampFormat string) {
	...
	// file_info := fileInfo(50);
	// fmt.Fprintf(b, "[%s]", file_info);
	for _, k := range keys {
		v := entry.Data[k]
...
func fileInfo(callDepth int) string {
	...
	f := runtime.FuncForPC(pc[count-3])
	// file, line := f.FileLine(pc[count-3])	
	file, _ := f.FileLine(pc[count-3])	

	// fmt.Printf("len: %d\n", count);
	// for idx:=0; idx < count; idx++{
	// 	f := runtime.FuncForPC(pc[idx])
	// 	file, line := f.FileLine(pc[idx])		
	// 	fmt.Printf("[%d]%s:%d\n", idx, file, line);
	// }
