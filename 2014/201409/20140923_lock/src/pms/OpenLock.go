...
func (this *OpenLock_REQ)Process(){
	fmt.Printf("数据： %v\n", this);
	
	//todo 入库
	var cmd proto.Remote_open_door;
	cmd.New();
	...
	cmd.Command.Head().LockID = (uint8)(lock);
	...
	if val, ok := manage.Works().Work_map[idx]; ok{
		...
		manage.Works().Work_map[idx].Task <- cmd;
	}else{
		fmt.Printf("对象盒子[%s]不在线%X\n", this.MidId, (uint32)(box));
		return;
	}

	// pack, _ := cmd.Encode();
	// proto.Hex_Dump(pack, len(pack));	
}
