...
func (this *Worker)Health(ID uint32, Conn net.Conn){
	defer func(){
		if err := recover(); err != nil{
			log.Warningf("检查连接健康失败, %v\n", err);
		}
		return;
	}();
	if val, ok := this.Work_map[ID]; ok{
		if val.Conn == Conn{
			log.V(99).Infof("旧连接存在而且正常[%v]\n", val.Conn);
			return;
		}else{
			log.Warningf("新旧连接不一致，关闭旧连接，登记新连接\n");
			this.Lost(Conn);
		}
	}
	this.Push(ID, Conn);
