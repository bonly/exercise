...
func (this *Connect)close(){
	...
	// 	defer func(){
	// 		if err := recover(); err != nil{
	// 			log.Warningf("关闭连接的注册通道失败, %v\n", err);
	// 		}	
	// 		wg.Done();
	// 		return;
	// 	}();
	// 	close(this.reg);//关闭注册通道
	// 	log.Warningf("关闭连接的注册通道\n");
	// }();
	wg.Wait();
...
func (this *Connect)Handle_connect(cbc chan interface{}){
	defer func(){
		if err := recover(); err != nil{
			log.Warningf("处理连接失败, %v\n", err);
			return;
		}
	}();	
...
func (this *Connect)recv_Msg(conn net.Conn, wg *sync.WaitGroup)(err error){
	...
	go func(){
		...
		defer func(){
			if err := recover(); err != nil{
				log.Warningf("处理消息失败, %v\n", err);
				return;
			}
		}();		
		defer wg.Done();
		buf := make([]byte, CMD_MAX_LEN);
		for this.run{
			// if *Test_mode {
			...
			if err != nil{
				log.Warningf("读取包头失败 %s\n", err.Error());
				return;
			}
			log.V(99).Infof("收到数据包Len[%d]\n", hl_len);

			if hl_len == 5{  //==5的是注册盒子
				var box Register;
				box.Decode(buf);
				log.V(99).Info("收到盒子心跳: ");
				...
				cmd := zh.Decode(buf);
				if cmd == nil{
					log.Warningf("收到错误的数据包\n");
					return;
				}
				log.V(99).Infof("收到协议数据包: \n");
				Hex_Dump(buf[:hl_len], hl_len);
				this.task <-zh;
			}
		}
				...
				case work, ok := <- this.task:{
					...
					switch {
						...
						case reflect.DeepEqual(cmd_param, LOCK_UPLOAD_LOG_CMD_PARAM):{
							...
							answer.(*Lock_upload_log).MidId = 
								fmt.Sprintf("%08X", Works().Conn_map[conn].ID);
						...
						case reflect.DeepEqual(cmd_param, USER_ADD_PASSWD_ID_CMD_PARAM):{
							answer = &R_User_add_passwd_id{};
							break;
						}							
					...
					if resp != nil{  //需要应答盒子的，说明是门锁主动的请求
						//回复门锁
