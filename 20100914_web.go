...
func get_mission_count(db *(mysql.Conn))(sh [500]MissionCnt) {
  ...
  `;
	...
	for i, row := range rows {
		mission[i].Chapter = row.Int(0);
		mission[i].Segment = row.Int(1);
		mission[i].Name = row.Str(2);
		mission[i].Count = row.Int(3);
	}
	return mission;
}
