...
string setup("t_need(2011)|t_need(3011)&a_need(1400)");
...
int main(){
  mydata md;
  rule<> r_single = str_p("t_need") >> ("(" >> int_p[myop(md)] >> ")")[myfunc(md)] | 
                    str_p("a_need") >> ("(" >> int_p[myop(md)] >> ")")[myfunc(md)] |
                    str_p("d_need") >> ("(" >> int_p[myop(md)] >> ")")[myfunc(md)] ;
  rule<> r_op = ch_p('|')|ch_p('&');                   
