...
func main(){
    ...
    //解base64密钥
    enbyte, err := base64.StdEncoding.DecodeString("fNCrhSynUm4=");
    if err != nil {  
	...
	key := []byte(enbyte); 
	//key := []byte("sfe023f_"); 
	crytext, err := DesEncrypt([]byte(cardstr), key[0:8]);
	//crytext, err := TripleDesEncrypt([]byte(cardstr), key);
	if err != nil {
		panic(err);
	}
	data.CardInfo=string(base64.StdEncoding.EncodeToString(crytext));
