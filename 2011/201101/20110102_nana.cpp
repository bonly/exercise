...
public:
    void arun(){
        ...
        rx = false;
        frm = new form; //form的生成show 及exec需在一起
...
int main(){
    ...
    boost::thread ot([&](){
        while(not_end){
            if(gui->frm&&gui->bx==nullptr){
                button *btn=new button(*(gui->frm), nana::rectangle(5, 5, 94, 23));
                btn->caption(STR("开始"));

                button oth(*(gui->frm), nana::rectangle(120, 5, 94, 23));
                button *bn2=nullptr;
                oth.make_event<events::click>([&](){
                    bn2 = new button(*(gui->frm), nana::rectangle(250, 5, 94, 23));
                });
                oth.caption(STR("新建"));

                gui->bx = new textbox(*(gui->frm), nana::rectangle(5, 40, 100, 100));
                ...
                gui->frm->make_event<events::destroy>([&](){ not_end = false;});
            }
            if(gui->rx){
