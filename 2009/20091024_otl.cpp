 ...
 */
 //clog << "rpc: " << i1.get_rpc() << endl;
 if(i1.get_rpc()<=0||i1.eof())
...
int main()
{
 ...
 try{
  ...
  db.rlogon("account/123@ocstest"); // connect to the database
#ifdef OTL_STREAM_POOLING_ON
  ...
  for(int i=1;;++i){
    ...
    select(i1); // select records from table
    //sleep(10);
  }
 }
