...
void select()
{
 ...
 otl_stream i(50, // buffer size
  ...
  otl_stream i(50,
        "select sessionId,"
                "createTime :#2<bigint>,taskID :#3<bigint,out>,subscriberID :#4<bigint,out>,"
                "accountID :#5<bigint,out>,accNbr :#6<bigint,out> "
                " from CreditTask "
                " where state=:st<char[3],in>", db);
  /**
   * 重载类型的字段以#编号,值为在字段中的序号,否则报:
   * Incompatible data types in stream operation
   * Column: 6<DOUBLE>, datatype in operator <</>>: CHAR
   */
  i<<"0";
  while(!i.eof())
  {
    char session[255]="";
    OTL_BIGINT createTime=0;
    OTL_BIGINT taskid=0;
    OTL_BIGINT subscriberID=0;
    OTL_BIGINT accountid=0;
    OTL_BIGINT accnbr=0;

    i >> session >> createTime  >> taskid >> subscriberID >> accountid >> accnbr;
    cerr << session << endl << createTime << endl << taskid << endl <<subscriberID << endl
          << accountid << endl << accnbr << endl;
  }
/*
 OTL_BIGINT f1;
 char f2[31];
...
int main()
{
 ...
 try{
  ...
  db.rlogon("account/123@ocstest"); // connect to Oracle
