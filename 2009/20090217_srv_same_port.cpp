...
class Srv
{
	...
	private:
		...
		void handle_receive(const boost::system::error_code& error,size_t)
		{
			...
			{
				cerr << "recv from "<<_remote_endpoint.address() <<":"
				     << _remote_endpoint.port()<<" : "<<_recv_buffer <<endl;
				shared_ptr<string> message(new string(_recv_buffer));
...
void run()
{
	...
	for (;;)
	{
		...
		cerr << "recv from "<<ep.address() <<": "<<buf <<endl;
