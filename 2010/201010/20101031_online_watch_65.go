...
func get_all_reg(db *(sql.DB)) int {
	sql := "select count(customer_id) from customer.customer"
	return exec_sql(sql, db)
}
...
func main() {
	...
	srv1.addr = flag.String("a", "127.0.0.1:3306", "mysql address")
	srv1.user = flag.String("u", "mysql", "mysql user name")
	srv1.pass = flag.String("p", "4860e49a", "mysql password")
