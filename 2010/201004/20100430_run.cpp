...
int wmain(int argc, WCHAR **argv)
{
    ...
    WCHAR lpszUsername[] = L"Administrator"; // Valid username
    WCHAR lpszDomain[] = L"."; // Using local user account database
    WCHAR lpszPassword[] = L"hayes"; // Johnny's password
    WCHAR lpCommandLine[] = L"C:\\Windows\\System32\\calc.exe";
    BOOL RetVal;

    // Call StartInteractiveClientProcess()
    RetVal = StartInteractiveClientProcess((LPTSTR) lpszUsername, // client to log on
                (LPTSTR) lpszDomain, // domain of client's account
                (LPTSTR) lpszPassword, // client's password
                (LPTSTR) lpCommandLine // command line to execute
                );
    wprintf((wchar_t*) "StartInteractiveClientProcess() returns %d\n", RetVal);
    return 0;
}
            ...
            )
{
    ...
    hwinsta = OpenWindowStation((LPCSTR) "winsta0", // the interactive window station
    ...
    hdesk = OpenDesktop(
                (LPCSTR) "default", // the interactive window station
                0, // no interaction with other desktop processes
                FALSE, // handle is not inheritable
                READ_CONTROL | // request the rights to read and write the DACL
                            WRITE_DAC | DESKTOP_WRITEOBJECTS
                            | DESKTOP_READOBJECTS);
    ...
    bResult = CreateProcessAsUser(hToken, // client's access token
