...
void wooinit(void) {
        pthread_create_orig = (int (*)(pthread_t *__restrict thread,
  __const pthread_attr_t *__restrict attr,
  void * (*start_routine)(void *),
  void *__restrict arg))dlsym(RTLD_NEXT, "pthread_create");  //网上流传的代码在此句都是错误的
        fprintf(stderr, "pthreads: using profiling hooks for gprof\n");
        if(pthread_create_orig == NULL){
                char *error = dlerror();
                if(error == NULL)         { error = "pthread_create is NULL";  }
                fprintf(stderr, "%s\n", error);
                exit(EXIT_FAILURE);
        }
}
...
static void * wrapper_routine(void * data) {     /* Put user data in thread-local variables */
  ...
  void *__restrict arg)
{
  ...
  i_return = pthread_create_orig(thread,attr,&wrapper_routine,&wrapper_data);  /* If the thread was successfully spawned, wait for the data      * to be released */
  if(i_return == 0)     {
